...
switch (evt) 
	...
	if(modbus_tcp_unit==MODBUS_ADRESS)
		...
		if(modbus_tcp_func==4)		//чтение произвольного кол-ва регистров	входов
			...
			{
			modbus_tcp_plazma[2]++;
			modbus_hold_register_write(modbus_tcp_unit,modbus_tcp_func,modbus_tcp_rx_arg0,modbus_tcp_rx_arg1, MODBUS_TCP_PROT);
	...
	/*		if(modbus_tcp_rx_arg0==11)		//Установка времени 
			...
			if(modbus_tcp_rx_arg0==20)		//Выходное напряжение системы 
				{
				U_OUT_SET=(uint16_t)modbus_tcp_rx_arg1;
