...
if((++cnt_net_drv>max_net_slot)&&(kan_aktivity_cnt))
	...
	if(KAN_BR==62)
		...
		{
		mcp2515_transmit(0xf1,(char)U_OUT_SET,(char)U_OUT_MIN,(char)(U_OUT_MAX-50), (char)U_NET_ON_MIN, (char)U_NET_OFF_MIN,(char)U_BAT_MIN,(char)U_BAT_MAX);
		mcp2515_transmit(0xf2,(WORK_FROM_MAIN==1) ? 1 : 0,(char)U_NET_OFF_MIN,(char)U_NET_ON_MIN,(char)(U_NET_ON_MAX-50),(char)(U_NET_OFF_MAX-50),0,0);
		}
	} 
...
if(language){ //o_2   
		...
		if((av_data_off[0]=='A')&&(av_data_off[1]=='A'))
			{
if(language)ptrs[3]="    активировано    ";//o_2
else 		ptrs[3]="     activated      ";//o_2
			bgnd_par(ptrs[0],ptrs[1],ptrs[2],ptrs[3]);
			}
		else 
			{
			gran_char(&index_set,0,1);
if(language)ptrs[3]="   деактивировано   ";//o_2
else		ptrs[3]="    deactivated     ";//o_2
			ptrs[4]="  0[]  0< 0>:0=:0z  ";	 /**/
			bgnd_par(ptrs[index_set],ptrs[1+index_set],ptrs[2+index_set],ptrs[3+index_set]);
			int2lcd(av_data_off[4],'>',0);
			int2lcd(av_data_off[5],'=',0);
...
if(language){ //o_2   
		ptrs[0]=" Ручн упр байпасом  ";
		ptrs[1]="работа только от инв";
}else{//o_2_s
		ptrs[0]=" Hand control bypass";
		ptrs[1]=" Work only from inv ";
}//o_2_e 
		ptrs[2]="  0%&  0^ 0@:0#:0$  ";	
		//ptrs[3]="                    ";
		if((av_data_off[0]=='A')&&(av_data_off[1]=='A'))
...
//o_2_e	
	...
	{	
if(language){  //o_2 
	ptrs[0]=" Инв 24В     CAN125 ";
	ptrs[1]=" Инв 48(60)В CAN125 ";
	ptrs[2]=" Инв 110В    CAN125 ";
	ptrs[3]=" Инв 220В    CAN125 ";
	ptrs[4]=" Инв 24В     CAN62.5";
	ptrs[5]=" Инв 48(60)В CAN62.5";
	ptrs[6]=" Инв 110В    CAN62.5";
	ptrs[7]=" Инв 220В    CAN62.5";
	ptrs[8]=sm_exit;
...
//o_2_s
	...
	ptrs[0]=" Inv 24V     CAN125 ";
	ptrs[1]=" Inv 48(60)V CAN125 ";
	ptrs[2]=" Inv 110V    CAN125 ";
	ptrs[3]=" Inv 220V    CAN125 ";
	ptrs[4]=" Inv 24V     CAN62.5";
	ptrs[5]=" Inv 48(60)V CAN62.5";
	ptrs[2]=" Inv 110V    CAN62.5";
	ptrs[7]=" Inv 220V    CAN62.5";
	ptrs[8]=" Exit               ";
	if(bFL5)ptrs[default_temp]=sm_;
...
/*		int2lcdyx(mat_temper,0,7,0);
		...
		int2lcdyx(byps[1]._unet_av_stat,2,19,0);
		...
		int2lcdyx(inv[7]._buff[17],1,19,0);	*/
     	...
     	bgnd_par(	"и1               и15",
     		    	...
     		    	"                    ",
     		    	"                    ");
     		int2lcdyx(plazma_can_time,0,6,0);
			    
			int2lcdyx(plazma_can_gl[0],1,3,0);
			int2lcdyx(plazma_can_gl[1],2,3,0);
			int2lcdyx(plazma_can_gl[2],3,3,0);

			int2lcdyx(plazma_can_gl[3],1,7,0);
			int2lcdyx(plazma_can_gl[4],2,7,0);
			int2lcdyx(plazma_can_gl[5],3,7,0);

			int2lcdyx(plazma_can_gl[6],1,11,0);
			int2lcdyx(plazma_can_gl[7],2,11,0);
			int2lcdyx(plazma_can_gl[8],3,11,0);

			int2lcdyx(plazma_can_gl[9],1,15,0);
			int2lcdyx(plazma_can_gl[10],2,15,0);
			int2lcdyx(plazma_can_gl[11],3,15,0);

			int2lcdyx(plazma_can_gl[12],1,19,0);
			int2lcdyx(plazma_can_gl[13],2,19,0);
			int2lcdyx(plazma_can_gl[14],3,19,0);
    		}
     	...
     	bgnd_par(	"и16              и30",
			...
			int2lcdyx(plazma_can_gl[15],1,3,0);
			int2lcdyx(plazma_can_gl[16],2,3,0);
			int2lcdyx(plazma_can_gl[17],3,3,0);

			int2lcdyx(plazma_can_gl[18],1,7,0);
			int2lcdyx(plazma_can_gl[19],2,7,0);
			int2lcdyx(plazma_can_gl[20],3,7,0);

			int2lcdyx(plazma_can_gl[21],1,11,0);
			int2lcdyx(plazma_can_gl[22],2,11,0);
			int2lcdyx(plazma_can_gl[23],3,11,0);

			int2lcdyx(plazma_can_gl[24],1,15,0);
			int2lcdyx(plazma_can_gl[25],2,15,0);
			int2lcdyx(plazma_can_gl[26],3,15,0);

			int2lcdyx(plazma_can_gl[27],1,19,0);
			int2lcdyx(plazma_can_gl[28],2,19,0);
			int2lcdyx(plazma_can_gl[29],3,19,0);
     	...
     	bgnd_par(	"и31              и33",
			...
			int2lcdyx(plazma_can_gl[30],1,3,0);
			int2lcdyx(plazma_can_gl[31],2,3,0);
			int2lcdyx(plazma_can_gl[32],3,3,0);
...
/*		int2lcdyx(modbus_rtu_plazma[0],1,4,0);
		...
		int2lcdyx(inv[27]._T,1,10,0);

		char2lcdhyx(inv[27]._buff[6],1,2);
...
//FullCAN_SetFilter(2,0x18e);
		...
		if(but==butE)
			...
			if(!plazma_stop) 
				...
				{
				char j;
				for(j=0; j<33; j++) plazma_can_gl[j]=0;
				plazma_stop=0;
				plazma_can_time=0;
				}
			}
...
#define SIMAXIDEF 8
	...
	if(but==butD)
		...
		if(sub_ind==0)
			...
			lc640_write_int(EE_KAN_BR,125);
			lc640_write_int(EE_WORK_FROM_MAIN,0);
			}
			...
			lc640_write_int(EE_U_BAT_MAX,23);
			lc640_write_int(EE_U_BAT_MIN,20);
			lc640_write_int(EE_U_NET_ON_MAX,248);
			lc640_write_int(EE_U_NET_OFF_MAX,253);
			lc640_write_int(EE_U_NET_ON_MIN,187);
			lc640_write_int(EE_U_NET_OFF_MIN,182);
...
if(++t0cnt>=10)
     ...
     if(++t0cnt3>=100)
		 ...
		 if(!plazma_stop)plazma_can_time++;
	     }
     }
