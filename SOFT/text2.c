...
#define CONST_AV_ACOUT_HNDL_MAX_CNT	50
signed char	acout_av_cnt,acout_av_cnt_A, acout_av_cnt_B, acout_av_cnt_C;
char acout_av_stat,acout_av_stat_A,acout_av_stat_B,acout_av_stat_C;
...
for	(ii_=0;ii_<2;ii_++)
	...
	if((wrkFromNet1Inv0)&&(RELE_SET_MASK[ii_]&0x10))  //если система работает от инверторов и включен флажок воздействия этого сигнала на это реле
		{
		avar_stat_temp[ii_]=1;
		}
	}
...
signed short /*temp_SS1_3F[3],*/temp_SS2_3F[3]/*,temp_SS3_3F[3]*/;
signed long temp_SL1_3F[3]/*,temp_SL2_3F[3]*/,temp_SL3_3F[3];
...
num_of_wrks_inv=0;
num_of_wrks_inv_3F[0]=0;
num_of_wrks_inv_3F[1]=0;
num_of_wrks_inv_3F[2]=0;

for(i=0;i<NUMINV;i++)
	{
	if((inv[i]._cnt<5/*_flags_tm&0x20*/) && (inv[i]._flags_tm&0x20)  &&  (NUMINV>i))
		{
		num_of_wrks_inv++;
		...
		if((i%3)==0)
			{
			num_of_wrks_inv_3F[0]++;
			temp_SL1_3F[0]+=(signed long)inv[i]._Uload;
			temp_SS2_3F[0]+=inv[i]._Iout;
			temp_SL3_3F[0]+=(signed long)inv[i]._Pout;
			}
		else if((i%3)==1)
			{
			num_of_wrks_inv_3F[1]++;
			temp_SL1_3F[1]+=(signed long)inv[i]._Uload;
			temp_SS2_3F[1]+=inv[i]._Iout;
			temp_SL3_3F[1]+=(signed long)inv[i]._Pout;
			}
		else if((i%3)==2)
			{
			num_of_wrks_inv_3F[2]++;
			temp_SL1_3F[2]+=(signed long)inv[i]._Uload;
			temp_SS2_3F[2]+=inv[i]._Iout;
			temp_SL3_3F[2]+=(signed long)inv[i]._Pout;
			}
		}
...
for(i=0;i<NUMINV;i++)
	{
	if((inv[i]._cnt<5)&&(inv[i]._flags_tm&0x20)&&(NUMINV>i))
		{
		temp_SL1+=(signed long)inv[i]._Uload;
		num_of_wrks_inv++;
		}
	}
//temp_SL3=-100L;
load_U_inv=(signed short)(temp_SL1/num_of_wrks_inv);
if(temp_SS2<0)temp_SS2=0;
load_I_inv=temp_SS2;
if(temp_SL3<0)temp_SL3=0;
load_P_inv=temp_SL3;

load_U_inv_3F[0]=(signed short)(temp_SL1_3F[0]/num_of_wrks_inv_3F[0]);
if(temp_SS2_3F[0]<0)temp_SS2_3F[0]=0;
load_I_inv_3F[0]=temp_SS2_3F[0];
if(temp_SL3_3F[0]<0)temp_SL3_3F[0]=0;
load_P_inv_3F[0]=temp_SL3_3F[0];

load_U_inv_3F[1]=(signed short)(temp_SL1_3F[1]/num_of_wrks_inv_3F[1]);
if(temp_SS2_3F[1]<0)temp_SS2_3F[1]=0;
load_I_inv_3F[1]=temp_SS2_3F[1];
if(temp_SL3_3F[1]<0)temp_SL3_3F[1]=0;
load_P_inv_3F[1]=temp_SL3_3F[1];

load_U_inv_3F[2]=(signed short)(temp_SL1_3F[2]/num_of_wrks_inv_3F[2]);
if(temp_SS2_3F[2]<0)temp_SS2_3F[2]=0;
load_I_inv_3F[2]=temp_SS2_3F[2];
if(temp_SL3_3F[2]<0)temp_SL3_3F[2]=0;
load_P_inv_3F[2]=temp_SL3_3F[2];

if((NUMPHASE==2)&&(NUMBYPASS==0))
	{
	char i, i_;
	signed short temp_SS;
	load_P_inv_3F[0]=(inv[0]._Pout+inv[1]._Pout);
	load_P_inv_3F[1]=(inv[2]._Pout+inv[3]._Pout+inv[4]._Pout+inv[5]._Pout);
	load_I_inv_3F[0]=(inv[0]._Iout+inv[1]._Iout);
	load_I_inv_3F[1]=(inv[2]._Iout+inv[3]._Iout+inv[4]._Iout+inv[5]._Iout);
	temp_SS=0;
	i_=0;
	for(i=0;i<2;i++)
		{
		if( (inv[i]._flags_tm&0x20) && (inv[i]._cnt<5)&&(NUMINV>i) )
			{
			temp_SS+=inv[i]._Uload;
			i_++;
			}
		}
	load_U_inv_3F[0]=temp_SS/(short)i_;
	temp_SS=0;
	i_=0;
	for(i=2;i<6;i++)
		{
	...
	load_U_inv_3F[1]=temp_SS/(short)i_;
	}
...
for(ii=0;ii<NUMINV;ii++)
	...
	if( (!(inv[ii]._flags_tm&0xbf)) && (!(inv[ii]._flags_tm_dop&0x01)) && (inv[ii]._valid) )
		...
		{
		if(inv[ii]._self_err_av_cnt>0)
			{
			inv[ii]._self_err_av_cnt-=3;
			if((inv[ii]._self_err_av_cnt<=0)&&(inv[ii]._self_err_av_stat))
				{
				inv[ii]._self_err_av_stat=0;
				avar_inv_hndl(ii,'S',0,0);
				} 
			}
		else inv[ii]._self_err_av_cnt=0;
		}
	// Авария по выходным напряжениям инверторов
	if((inv[ii]._flags_tm&0x08) &&  inv[ii]._valid)
		{
		if(inv[ii]._uout_max_av_cnt<CONST_AV_INV_HNDL_MAX_CNT)
			{
			inv[ii]._uout_max_av_cnt++;
			if((inv[ii]._uout_max_av_cnt>=CONST_AV_INV_HNDL_MAX_CNT)&&(!inv[ii]._uout_av_stat))
				{
	...
	else if((!(inv[ii]._flags_tm&0x08)) &&  inv[ii]._valid)
		{
		if(inv[ii]._uout_max_av_cnt)
			{
			inv[ii]._uout_max_av_cnt--;
			if((inv[ii]._uout_max_av_cnt==0)&&(inv[ii]._uout_av_stat))
				{
	...
	if((inv[ii]._flags_tm&0x10) &&  inv[ii]._valid)
		{
		if(inv[ii]._uout_min_av_cnt<CONST_AV_INV_HNDL_MAX_CNT)
			{
			inv[ii]._uout_min_av_cnt++;
			if((inv[ii]._uout_min_av_cnt>=CONST_AV_INV_HNDL_MAX_CNT)&&(!inv[ii]._uout_av_stat))
				{
				inv[ii]._uout_av_stat=1;
				avar_inv_hndl(ii,'o',1,inv[ii]._Uout);
				}  
	...
	else if((!(inv[ii]._flags_tm&0x10)) &&  inv[ii]._valid)
		{
		if(inv[ii]._uout_min_av_cnt)
			{
			inv[ii]._uout_min_av_cnt--;
			if((inv[ii]._uout_min_av_cnt==0)&&(inv[ii]._uout_av_stat))
				{
				inv[ii]._uout_av_stat=0;
				avar_inv_hndl(ii,'o',0,0);
				} 
	...
	else if((!(inv[ii]._flags_tm&0x06)) &&  inv[ii]._valid)
		{
	...
	// Авария по перегрузке инверторов
	if((inv[ii]._flags_tm&0x01) &&  inv[ii]._valid)
		{
		if(inv[ii]._overload_av_cnt<CONST_AV_INV_HNDL_MAX_CNT)
			{
			inv[ii]._overload_av_cnt++;
			if((inv[ii]._overload_av_cnt>=CONST_AV_INV_HNDL_MAX_CNT)&&(!inv[ii]._overload_av_stat))
				{
				inv[ii]._overload_av_stat=1;
				avar_inv_hndl(ii,'L',1,0);
				}  
			}
		}  
	else if((!(inv[ii]._flags_tm&0x01)) &&  inv[ii]._valid)
		{
		if(inv[ii]._overload_av_cnt)
			{
			inv[ii]._overload_av_cnt--;
			if((inv[ii]._overload_av_cnt==0)&&(inv[ii]._overload_av_stat))
				{
				inv[ii]._overload_av_stat=0;
				avar_inv_hndl(ii,'L',0,0);
				} 
			}
		}
...
//inv[sub_ind1]._flags_tm_dop&0x01

	// Авария по входному напряжению
	if((inv[ii]._flags_tm_dop&0x01)&&(inv[ii]._valid))
		{
		if(inv[ii]._uin_av_cnt<CONST_AV_INV_HNDL_MAX_CNT)
			{
			inv[ii]._uin_av_cnt++;
			if((inv[ii]._uin_av_cnt>=CONST_AV_INV_HNDL_MAX_CNT)&&(!inv[ii]._uin_av_stat))
				{
				inv[ii]._uin_av_stat=1;
				avar_inv_hndl(ii,'I',1,0);
				}  
			}
		}  
	else if(inv[ii]._valid)
		{
		if(inv[ii]._uin_av_cnt)
			{
			inv[ii]._uin_av_cnt--;
			if((inv[ii]._uin_av_cnt==0)&&(inv[ii]._uin_av_stat))
				{
				inv[ii]._uin_av_stat=0;
				avar_inv_hndl(ii,'I',0,0);
				} 
			}
		}
...
for(ii=0;ii<NUMINV;ii++)
	...
	if
		(
		(inv[ii]._uout_av_stat) ||
		(inv[ii]._temper_av_stat) ||
		(inv[ii]._conn_av_stat) ||
...
if(temp_==0)
	{
	for(ii=0;ii<NUMBYPASS;ii++) if(byps[ii]._cnt<5) temp_=1;
	}
systemIsWrk = temp_;
...
/*	temp=0;
	...
	if(NUMPHASE==1)
		{
		if(acout_av_stat)uOutAvIsOn=1;
		else uOutAvIsOn=0;
		}
	else if(NUMPHASE==2)
		{
		if(acout_av_stat_A||acout_av_stat_B)uOutAvIsOn=1;
		else uOutAvIsOn=0;
		}
	else if(NUMPHASE==3)
		{
		if(acout_av_stat_A||acout_av_stat_B||acout_av_stat_C)uOutAvIsOn=1;
		else uOutAvIsOn=0;
		}
	}
else
...
if(wrkFromNet1Inv0)
	...
	{
	if(wrkFromNet1Inv0_cnt>0)
		{
		wrkFromNet1Inv0_cnt--;
		if(wrkFromNet1Inv0_cnt==0) 
			{
			if(wrkFromNet1Inv0_del==1)
				{
				
				wrk_from_inv_hndl();
				}
			wrkFromNet1Inv0_del=0;
			}
		}
	else wrkFromNet1Inv0_cnt=0;
	}
...
if(priorFromNet1Inv0)
	...
	{
	if(priorFromNet1Inv0_cnt>0)
		{
		priorFromNet1Inv0_cnt--;
		if(priorFromNet1Inv0_cnt==0) 
			{
			if(priorFromNet1Inv0_del==1)
				{
				
				prior_from_inv_hndl();
				}
			priorFromNet1Inv0_del=0;
			}
		}
...
//-----------------------------------------------
void ac_out_av_hndl(void)
{
// Авария по выходному напряжению AC (без байпаса)

if(NUMBYPASS) return;
if(NUMPHASE==1)
	{
	if((load_U_inv/10>U_OUT_AC_MAX_AV/*U_OUT_MAX*/)||(load_U_inv/10<U_OUT_AC_MIN_AV/*U_OUT_MIN*/))
		{
		if(acout_av_cnt<CONST_AV_ACOUT_HNDL_MAX_CNT)
			{
			acout_av_cnt++;
			if((acout_av_cnt>=CONST_AV_ACOUT_HNDL_MAX_CNT)&&(!acout_av_stat))
				{
				acout_av_stat=1;
				avar_acout_hndl(1,load_U_inv/10,0);
				}
			}  
		}
	else if((load_U_inv/10<=U_OUT_AC_MAX_AV/*U_OUT_MAX*/)&&(load_U_inv/10>=U_OUT_AC_MIN_AV/*U_OUT_MIN*/))
		{
		if(acout_av_cnt)
			{
			acout_av_cnt--;
			if((acout_av_cnt==0)&&(acout_av_stat))
				{
				acout_av_stat=0;
				avar_acout_hndl(0,0,0);
				}
			}  
		}
	}

if((NUMPHASE==2)||(NUMPHASE==3))
	{
	if((load_U_inv_3F[0]/10>U_OUT_AC_MAX_AV/*U_OUT_MAX*/)||(load_U_inv_3F[0]/10<U_OUT_AC_MIN_AV/*U_OUT_MIN*/))
		{
		if(acout_av_cnt_A<CONST_AV_ACOUT_HNDL_MAX_CNT)
			{
			acout_av_cnt_A++;
			if((acout_av_cnt_A>=CONST_AV_ACOUT_HNDL_MAX_CNT)&&(!acout_av_stat_A))
				{
				acout_av_stat_A=1;
				avar_acout_hndl(1,load_U_inv_3F[0]/10,1);
				}
			}  
		}
	else if((load_U_inv_3F[0]/10<=U_OUT_AC_MAX_AV/*U_OUT_MAX*/)&&(load_U_inv_3F[0]/10>=U_OUT_AC_MIN_AV/*U_OUT_MIN*/))
		{
		if(acout_av_cnt_A)
			{
			acout_av_cnt_A--;
			if((acout_av_cnt_A==0)&&(acout_av_stat_A))
				{
				acout_av_stat_A=0;
				avar_acout_hndl(0,0,1);
				}
			}  
		}

	if((load_U_inv_3F[1]/10>U_OUT_AC_MAX_AV/*U_OUT_MAX*/)||(load_U_inv_3F[1]/10<U_OUT_AC_MIN_AV/*U_OUT_MIN*/))
		{
		if(acout_av_cnt_B<CONST_AV_ACOUT_HNDL_MAX_CNT)
			{
			acout_av_cnt_B++;
			if((acout_av_cnt_B>=CONST_AV_ACOUT_HNDL_MAX_CNT)&&(!acout_av_stat_B))
				{
				acout_av_stat_B=1;
				avar_acout_hndl(1,load_U_inv_3F[1]/10,2);
				}
			}  
		}
	else if((load_U_inv_3F[1]/10<=U_OUT_AC_MAX_AV/*U_OUT_MAX*/)&&(load_U_inv_3F[1]/10>=U_OUT_AC_MIN_AV/*U_OUT_MIN*/))
		{
		if(acout_av_cnt_B)
			{
			acout_av_cnt_B--;
			if((acout_av_cnt_B==0)&&(acout_av_stat_B))
				{
				acout_av_stat_B=0;
				avar_acout_hndl(0,0,2);
				}
			}  
		}
	}
...
if((NUMPHASE==3))
	{
	if((load_U_inv_3F[2]/10>U_OUT_AC_MAX_AV/*U_OUT_MAX*/)||(load_U_inv_3F[2]/10<U_OUT_AC_MIN_AV/*U_OUT_MIN*/))
		{
		if(acout_av_cnt_C<CONST_AV_ACOUT_HNDL_MAX_CNT)
			{
			acout_av_cnt_C++;
			if((acout_av_cnt_C>=CONST_AV_ACOUT_HNDL_MAX_CNT)&&(!acout_av_stat_C))
				{
				acout_av_stat_C=1;
				avar_acout_hndl(1,load_U_inv_3F[2]/10,3);
				}
			}  
		}
	else if((load_U_inv_3F[2]/10<=U_OUT_AC_MAX_AV/*U_OUT_MAX*/)&&(load_U_inv_3F[2]/10>=U_OUT_AC_MIN_AV/*U_OUT_MIN*/))
		{
		if(acout_av_cnt_C)
			{
			acout_av_cnt_C--;
			if((acout_av_cnt_C==0)&&(acout_av_stat_C))
				{
				acout_av_stat_C=0;
				avar_acout_hndl(0,0,3);
				}
			}  
		}
	}
...
signed short U_NET_ON_MAX;
signed short U_NET_OFF_MAX;
signed short U_OUT_MAX;
signed short U_OUT_MIN;
signed short U_NET_ON_MIN;
...
char wrkFromNet1Inv0_del=10; 	//работа от сети(1)/инверторов(0) с задержкой, после счетчика
char wrkFromNet1Inv0_cnt; 	//работа от сети(1)/инверторов(0) счетчик
char priorFromNet1Inv0; 		//приоритет - работа от сети(1)/инверторов(0)
char priorFromNet1Inv0_del=10; 	//приоритет - работа от сети(1)/инверторов(0) с задержкой, после счетчика
char priorFromNet1Inv0_cnt; 	//приоритет - работа от сети(1)/инверторов(0) счетчик
char systemIsWrk;				//система работает, есть хотя бы один инвертор который передает по KAN что он подключен к выходной шине
char fBypsInAvIsOn;				//авария по расфазировке хотя бы у одного байпаса
char fBypsInvAvIsOn;			//авария по расфазировке входного инверторного напряжения хотя бы у одного байпаса
...
//o_2_s
//------  английский язык -----------------------
signed short language;
const char sm_mont_en[13][4]={"   ","jan","feb","mar","apr","may","jun","jul","aug","sep","oct","nov","dec"};

void mess_set_inv (void){
	if(language)
			show_mess(		"Изменение параметров",
	          				"   работы системы   ",
	          				"  заблокировано!!!  ",
	          				"                    ",3000);
	else
			show_mess(		"   Changing system  ",
...
if((++cnt_net_drv>max_net_slot)&&(kan_aktivity_cnt))
	{
	cnt_net_drv=0;
...
/*0502*/
		if(inv[cnt_net_drv-MINIM_INV_ADRESS]._cnt<CNT_SRC_MAX)
   	 		{    
   	 		inv[cnt_net_drv-MINIM_INV_ADRESS]._cnt++;

   	 		}
		else inv[cnt_net_drv-MINIM_INV_ADRESS]._cnt=CNT_SRC_MAX;
...
/*0702		ptrs[1]="    Pвых=     @Вт   ";	 0702*/
/*0702*/ptrs[1]="Pвых=   ]/  ]кВт    ";
 		ptrs[2]="Uвых=  [В/  zВ      ";
		ptrs[3]="Iвых=  }A/  }A      ";
     //	ptrs[2]="    Pвых=     @Вт   ";
     	ptrs[4]=" 0%:0^:0& 0</>  /0{ ";
...
/*0702*/ptrs[1]="Pвых=   ]/  ]/  ]кВт";
	...
	if((NUMPHASE==3)||(NUMPHASE==2))
		...
		if(NUMBYPASS)
			...
			{
			int2lcd(load_U_inv_3F[0]/10,'[',0);
			int2lcd(load_U_inv_3F[1]/10,'z',0);
			int2lcd(load_U_inv_3F[2]/10,'Z',0);

			/*0702		if(byps[0]._Iout>999)int2lcd(byps[0]._Iout/10,']',0);
     		else int2lcd(byps[0]._Iout,']',1);  
   			//int2lcd_mmm(byps._T,'[',0);	0702*/

			if(load_I_inv_3F[0]>99)int2lcd(load_I_inv_3F[0]/10,'}',0);
		    else int2lcd(load_I_inv_3F[0],'}',1); 
			if(load_I_inv_3F[1]>99)int2lcd(load_I_inv_3F[1]/10,'}',0);
		    else int2lcd(load_I_inv_3F[1],'}',1);
			if(load_I_inv_3F[2]>99)int2lcd(load_I_inv_3F[2]/10,'}',0);
		    else int2lcd(load_I_inv_3F[2],'}',1); 
...
/*0702*/if((NUMPHASE==3)||(NUMPHASE==2))
			...
			if(NUMBYPASS)
				...
				if(byps[1]._Pout>10000)	long2lcd_mmm((unsigned short)(byps[1]._Pout/1000),']',0);
				else 					long2lcd_mmm((unsigned short)(byps[1]._Pout/100),']',1);
				if(byps[2]._Pout>10000)	long2lcd_mmm((unsigned short)(byps[2]._Pout/1000),']',0);
				else 					long2lcd_mmm((unsigned short)(byps[2]._Pout/100),']',1);
				}
			else 
				{
				if(load_P_inv_3F[0]>10000)	long2lcd_mmm((unsigned short)(load_P_inv_3F[0]/1000),']',0);
				else 						long2lcd_mmm((unsigned short)(load_P_inv_3F[0]/100),']',1);
				if(load_P_inv_3F[1]>10000)	long2lcd_mmm((unsigned short)(load_P_inv_3F[1]/1000),']',0);
				else 						long2lcd_mmm((unsigned short)(load_P_inv_3F[1]/100),']',1);
				if(load_P_inv_3F[2]>10000)	long2lcd_mmm((unsigned short)(load_P_inv_3F[2]/1000),']',0);
				else 						long2lcd_mmm((unsigned short)(load_P_inv_3F[2]/100),']',1);
				}
...
/*0702*/ptrs[1]="Pout=   ]/  ]/  ]кW ";
	...
	if(flfl) ptrs[0]	=	"Check the structure ";
	...
	if((NUMPHASE==3)||(NUMPHASE==2))
		...
		if(NUMBYPASS)
			...
			{
			int2lcd(load_U_inv_3F[0]/10,'[',0);
			int2lcd(load_U_inv_3F[1]/10,'я',0);
			int2lcd(load_U_inv_3F[2]/10,'Я',0);
	...
	if(ND_EXT[0])sub_bgnd("error",'ы',-2);
	else int2lcd_mmm(t_ext[0],'ы',0);
...
if(language){//o_2
	...
	else if(((inv[sub_ind1]._flags_tm&0x22)==0x02) || ((inv[sub_ind1]._flags_tm&0x24)==0x04))
		{
...
//o_2_s
	...
	if(inv[sub_ind1]._cnt>5)
		...
		{
		ptr[0]=		"In operat. HIGH Uout";	      
		}
	else if((inv[sub_ind1]._flags_tm&0x30)==0x30)
		{
		ptr[0]=		"In operat. LOW Uout ";	      
		}
	else if((inv[sub_ind1]._flags_tm&0x24)==0x24)
		{
		...
		{
		ptr[0]=		"In operation on DC  ";	      
		}
	else if((inv[sub_ind1]._flags_tm&0xa0)==0xa0)
		{
		ptr[0]=		"In operation on AC  ";	      
		}
...
//o_2_e
	...
	//int2lcdyx(inv[sub_ind1]._cnt,0,19,0); 
	//int2lcdyx(inv[sub_ind1]._cnt,0,3,0); 
//int2lcdyx(ava,0,4,0);	
//int2lcdyx(plazma_inv[0],0,2,0);
//int2lcdyx(plazma_inv[1],0,5,0);
//int2lcdyx(plazma_inv[2],0,8,0);
...
//o_2_s
	...
	if(iByps_ind_cnt<=20)
		{
		if(byps[0]._flags&0x40)ptr[0]=				"Priority: invertors ";
		else ptr[0]=								"Priority: mains     ";
		}

	if(iByps_ind_cnt>20)
		{
		if(byps[0]._flags&0x80)ptr[0]=				"In operate: invertor";
		else ptr[0]=								"In operate: mains   ";
		}
...
/*0705	long2lcd_mmm((unsigned short)(byps[0]._Pout/100),']',1);
	long2lcd_mmm((unsigned short)(byps[1]._Pout/100),']',1);
	long2lcd_mmm((unsigned short)(byps[2]._Pout/100),']',1);	0705*/
/*0705*/
	if(byps[0]._Pout>10000)	long2lcd_mmm((unsigned short)(byps[0]._Pout/1000),']',0);
	else 					long2lcd_mmm((unsigned short)(byps[0]._Pout/100),']',1);
	if(byps[1]._Pout>10000)	long2lcd_mmm((unsigned short)(byps[1]._Pout/1000),']',0);
...
//o_2_s
	...
	temp=-11;
	if(sk_stat[0]==ssON)		temp=-9;
	if(sk_av_stat[0]==sasON) 	sub_bgnd("ALARM",'$',temp-2);
	else                     	sub_bgnd("NORM",'$',temp-1);
	if(sk_stat[0]==ssON)		sub_bgnd("CLOSED",'$',-5);
	if(sk_stat[0]==ssOFF)		sub_bgnd("OPEN  ",'$',-5);

	temp=-11;
	if(sk_stat[1]==ssON)		temp=-9;
	if(sk_av_stat[1]==sasON) 	sub_bgnd("ALARM",'%',temp-2);
	else                     	sub_bgnd("NORM",'%',temp-1);
	if(sk_stat[1]==ssON)		sub_bgnd("CLOSED",'%',-5);
	if(sk_stat[1]==ssOFF)		sub_bgnd("OPEN  ",'%',-5);

	temp=-11;
	if(sk_stat[2]==ssON)		temp=-9;
	if(sk_av_stat[2]==sasON) 	sub_bgnd("ALARM",'^',temp-2);
	else                     	sub_bgnd("NORM",'^',temp-1);
	if(sk_stat[2]==ssON)		sub_bgnd("CLOSED",'^',-5);
	if(sk_stat[2]==ssOFF)		sub_bgnd("OPEN  ",'^',-5);

	temp=-11;
	if(sk_stat[3]==ssON)		temp=-9;
	if(sk_av_stat[3]==sasON) 	sub_bgnd("ALARM",'&',temp-2);
	else                     	sub_bgnd("NORM",'&',temp-1);
 	if(sk_stat[3]==ssON)		sub_bgnd("CLOSED",'&',-5);
	if(sk_stat[3]==ssOFF)		sub_bgnd("OPEN  ",'&',-5);
	}
//o_2_e
     }
...
//o_2_s
	...
	if(av_j_si_max==0)
		...
		if(index_set==0) bgnd_par(	"     EVENT LOG      ",
									" (                  ",
									" [                  ",
									" Exit               ");
		else if(index_set==1) bgnd_par(	"     EVENT LOG      ",
										" (                  ",
										" Exit               ",
										" Clear event log    ");
		}
		...
		if(index_set==(av_j_si_max-1)) bgnd_par(	"     EVENT LOG      ",
													" (                  ",
													" Exit               ",
													" Clear event log    ");
		else if(index_set==(av_j_si_max-2)) bgnd_par(	"     EVENT LOG      ",
														" (                  ",
														" [                  ",
														" Exit               ");
		else bgnd_par(	"     EVENT LOG      ",
						" (                  ",
						" [                  ",
						" {                  ");
		}
...
if(language)	bgnd_par(	//o_2
		...
		if(language)sub_bgnd(sm_mont[av_data_on[1]],'(',0);//o_2
		else		sub_bgnd(sm_mont_en[av_data_on[1]],'(',0);//o_2
	...
	if((av_head[0]=='W')&&(av_head[2]=='N'))
		{
if(language)	bgnd_par(	//o_2
				"   Переход работы   ",
				"      байпаса       ",
				"с инверторов на сеть",
				"  0%(  0^ 0@:0#:0$  ");
else 			bgnd_par(	//o_2
				"  Switching bypass  ",//o_2
				"   operation from   ",//o_2
				" inverters to mains ",//o_2
				"  0%(  0^ 0@:0#:0$  ");//o_2				
...
if(language)		bgnd_par(	//o_2
		...
		{  
if(language)		ptrs[0]="   Авария сети!!!   ";//o_2
else				ptrs[0]="   Alarm mains!!!   ";//o_2
		ptrs[1]="  0%(  0^ 0@:0#:0$  ";
		if((av_data_off[0]=='A')&&(av_data_off[1]=='A'))
			{
if(language){		ptrs[2]="    не устранена    ";//o_2
					ptrs[3]="     Uсети=  +В     ";}//o_2
else		{		ptrs[2]="    not resolved    ";//o_2
					ptrs[3]="     Umains=  +V    ";}//o_2
...
if(language)ptrs[2]="      устранена     ";//o_2
else		ptrs[2]="      resolved      ";//o_2
			ptrs[3]="  0[]  0< 0>:0=:0)  ";
if(language)ptrs[4]="     Uмин=  +В      ";//o_2
else		ptrs[4]="     Umin=  +V      ";//o_2
	...
	else if((av_head[0]=='O')&&(av_head[2]=='U'))
		{
if(language){//o_2  
		ptrs[0]=" Авария Uвых     !В ";
		ptrs[1]=" Uвых.  не в норме! ";
		if(av_head[1]==1)		ptrs[1]=" Uвых.фА не в норме!";
		if(av_head[1]==2)		ptrs[1]=" Uвых.фB не в норме!";
		if(av_head[1]==3)		ptrs[1]=" Uвых.фC не в норме!";
}else{//o_2_s
		ptrs[0]=" Alarm Uout      !V ";
		ptrs[1]=" Uout is not normal!";
		if(av_head[1]==1)		ptrs[1]="Uout А is not normal";
		if(av_head[1]==2)		ptrs[1]="Uout B is not normal";
		if(av_head[1]==3)		ptrs[1]="Uout C is not normal";
}//o_2_e
		ptrs[2]="  0%&  0^ 0@:0#:0$  ";
		if((av_data_off[0]=='A')&&(av_data_off[1]=='A'))
			{
if(language)ptrs[3]="    не устранена    ";//o_2
else 		ptrs[3]="    not resolved    ";//o_2
			bgnd_par(ptrs[0],ptrs[1],ptrs[2],ptrs[3]);
			}
		else 
			{
			gran_char(&index_set,0,1);
if(language)ptrs[3]="     устранена      ";//o_2
else		ptrs[3]="      resolved      ";//o_2
			ptrs[4]="  0[]  0< 0>:0=:0z  ";
			bgnd_par(ptrs[index_set],ptrs[1+index_set],ptrs[2+index_set],ptrs[3+index_set]);
			int2lcd(av_data_off[4],'>',0);
			int2lcd(av_data_off[5],'=',0);
...
if(language){ //o_2
		ptrs[0]=" Авария Udc      !В ";
		ptrs[1]=" Udc.вх не в норме! ";
}else{//o_2_s
		ptrs[0]=" Alarm Udc       !V ";
		ptrs[1]=" Udc is not normal! ";
}//o_2_e
		ptrs[2]="  0%&  0^ 0@:0#:0$  ";
		if((av_data_off[0]=='A')&&(av_data_off[1]=='A'))
			{
...
if(language)ptrs[3]="     устранена      ";//o_2
	...
	else if((av_head[0]=='I')&&(av_head[2]=='S'))
		{
if(language){ //o_2  
		ptrs[0]=" Авария инвертор NZ ";
		ptrs[1]=" Внутр.неисправность";
}else{//o_2_s
		ptrs[0]=" Alarm inverter NZ  ";
		ptrs[1]="     Defective      ";
}//o_2_e
		ptrs[2]="  0%&  0^ 0@:0#:0$  ";
		if((av_data_off[0]=='A')&&(av_data_off[1]=='A'))
...
if(language)ptrs[3]="     устранена      ";//o_2
	...
	else if((av_head[0]=='I')&&(av_head[2]=='I'))
		{
if(language){ //o_2 
		ptrs[0]=" Авария инвертор NZ ";
		ptrs[1]=" Udc не в норме     ";
}else{//o_2_s
		ptrs[0]=" Alarm inverter NZ  ";
		ptrs[1]=" Udc is not normal  ";
}//o_2_e   
...
if(language)ptrs[3]="     устранена      ";//o_2
	...
	else if((av_head[0]=='I')&&(av_head[2]=='L'))
		{
if(language){ //o_2 
		ptrs[0]=" Авария инвертор NZ ";
		ptrs[1]=" Перегрузка по Pвых ";
}else{//o_2_s
		ptrs[0]=" Alarm inverter NZ  ";
		ptrs[1]="      Overload      ";
}//o_2_e  
...
if(language)ptrs[3]="     устранена      ";//o_2
		...
		{
if(language){ //o_2   
		ptrs[0]="   Авария байпас    ";
		ptrs[1]="    разрыв связи    ";
}else{//o_2_s
		ptrs[0]="   Alarm bypass     ";
		ptrs[1]="  connection lost   ";
}//o_2_e 
		ptrs[2]="  0%&  0^ 0@:0#:0$  ";
		if((av_data_off[0]=='A')&&(av_data_off[1]=='A'))
			{
...
if(language){ //o_2   
		...
		ptrs[0]=" Alarm Umains    !V ";
		ptrs[1]="  bypass phase Z    ";
}//o_2_e 
		ptrs[2]="  0%&  0^ 0@:0#:0$  ";
		if((av_data_off[0]=='A')&&(av_data_off[1]=='A'))
			{
...
if(language)ptrs[3]="     устранена      ";//o_2
	...
	else if((av_head[0]=='B')&&(av_head[2]=='F'))
		{
if(language){ //o_2   
		ptrs[0]=" Синхронизация с    ";
		ptrs[1]=" инв.-ми не в норме ";
}else{//o_2_s
		ptrs[0]=" Sinchr.-tion  with ";
		ptrs[1]="inv.-s is not normal";
}//o_2_e 
		ptrs[2]="  0%&  0^ 0@:0#:0$  ";
		if((av_data_off[0]=='A')&&(av_data_off[1]=='A'))
			{
...
if(language)ptrs[3]="     устранена      ";//o_2
	...
	else if((av_head[0]=='B')&&(av_head[2]=='M'))
		{
if(language){ //o_2   
		ptrs[0]=" Ручн упр байпасом  ";
		ptrs[1]="работа только от сет";
}else{//o_2_s
		ptrs[0]=" Hand control bypass";
		ptrs[1]="Work only from mains";
}//o_2_e 
		ptrs[2]="  0%&  0^ 0@:0#:0$  ";
/*		if((av_data_off[0]=='A')&&(av_data_off[1]=='A'))
			{
...
if(language){ //o_2   
		ptrs[0]=" Синхронизация с    ";
		ptrs[1]=" сетью не в норме   ";
}else{//o_2_s
		ptrs[0]=" Sinchr.-tion  with ";
		ptrs[1]=" mains is not normal";
}//o_2_e 
		ptrs[2]="  0%&  0^ 0@:0#:0$  ";
		if((av_data_off[0]=='A')&&(av_data_off[1]=='A'))
			{
...
if(language)ptrs[3]="     устранена      ";//o_2
	...
	{
	if(language) bgnd_par("  Введите  пароль   ",sm_,sm_,sm_);  //o_2
	else 		 bgnd_par("  Enter password    ",sm_,sm_,sm_);  //o_2
...
//o_2_s
	...
	ptrs[0]="  To clear the log, ";
	bgnd_par(ptrs[0],"  enter password    ",sm_,sm_);
}//o_2_e	
	...
	ptrs[7]=		"  аварии    )       ";
	ptrs[8]=		" Уставки инверторов ";
	ptrs[9]=		" Уставки байпаса    ";
/*	ptrs[10]=		" Напряжение выхода  ";
	...
	ptrs[10]=		" Ethernet           ";
	ptrs[11]=		" MODBUS ADRESS     <";
	ptrs[12]=		" MODBUS BAUDRATE    ";
	ptrs[13]=		"                  >0";
	ptrs[14]=      	" Реле               ";
	ptrs[15]=      	" Аварийные пороги   ";
	ptrs[16]=      	" системы            ";
	ptrs[17]=		" Внешние датчики    ";
	ptrs[18]=      	" Серийный N        w";
	ptrs[19]=      	" Индикация Fвых    W";
	ptrs[20]=		" Язык меню: Rus     ";//o_2
	ptrs[21]=		" Baudrate CAN       ";//o_2
	if(KAN_BR==62)	ptrs[22]=		"            62.5kbps";//o_2
	else 			ptrs[22]=		"             125kbps";//o_2
    ptrs[23]=		" Выход              ";//o_2
    ptrs[24]=		" Калибровки         ";//o_2 
    ptrs[26]=		"                    ";//o_2        
...
/*	int2lcdyx(sub_ind1,0,6,0);
	int2lcdyx(KAN_BR,0,12,0);*/

	//long2lcd_mmm(AUSW_MAIN_NUMBER,'w',0);
	serial2lcd(AUSW_MAIN_NUMBER,'w',sub_ind1);
	
	if(F_IND_EN)sub_bgnd("ВКЛ.",'W',-3);
	else sub_bgnd("ВЫК.",'W',-3);
/*
...
//o_2_s
	...
	ptrs[0]=		" Standard settings  ";
	ptrs[1]=		" Time and date      ";
	ptrs[2]=		" Time and date      ";
	ptrs[3]=		"   synchronization  ";
    ptrs[4]=		" Structure          ";
	ptrs[5]=		" Sound alarm     (  ";
	ptrs[6]=		" Alarm signal       ";
	ptrs[7]=		" deactivation )     ";
	ptrs[8]=		" Inverter Settings  ";
	ptrs[9]=		" Bypass Settings    ";
/*	ptrs[8]=		" Inverter output    ";
	ptrs[9]=		"   voltage        !V";
	ptrs[10]=		" Maximum output     ";
	ptrs[11]=		"     voltage      @V";
	ptrs[12]=		" Minimum output     ";
	ptrs[13]=		"     voltage      #V";
	ptrs[14]=		" Turn-on mains      ";
	ptrs[15]=		"     voltage      [V";
	ptrs[16]=		" Turn-off mains     ";
	ptrs[17]=		"     voltage      ]V";
	ptrs[18]=		" Turn-on battery    ";
	ptrs[19]=		"     voltage      {V";
	ptrs[20]=		" Turn-off battery   ";
	ptrs[21]=		"     voltage      }V";	*/
	ptrs[10]=		" Ethernet           ";
	ptrs[11]=		" MODBUS ADRESS     <";
	ptrs[12]=		" MODBUS BAUDRATE    ";
	ptrs[13]=		"                  >0";
	ptrs[14]=      	" Relay              ";
	ptrs[15]=      	" System alarm       ";
	ptrs[16]=      	"       thresholds   ";
	ptrs[17]=		" External sensors   ";
	ptrs[18]=      	" Serial No.        ч";
	ptrs[19]=      	" Display Fout      Ц";
	ptrs[20]=		" Language:  English ";
	ptrs[21]=		" Baudrate CAN       ";//o_2
	if(KAN_BR==62)	ptrs[22]=		"            62.5kbps";//o_2
	else 			ptrs[22]=		"             125kbps";//o_2
    ptrs[23]=		" Exit               ";
...
/*	if((index_set==9)||(index_set==13)||(index_set==17))
	...
	if(ZV_ON)sub_bgnd("ON",'(',0);
	else sub_bgnd("OFF",'(',0);
	if(AV_OFF_AVT)sub_bgnd("auto",')',0);
	else sub_bgnd("manual",')',0);
	int2lcd(MODBUS_ADRESS,'<',0);
	int2lcd(MODBUS_BAUDRATE,'>',0);
//	int2lcd(U_OUT_SET,'!',0);
//	int2lcd(U_OUT_MAX,'@',0);
...
//	int2lcd(U_BAT_MIN,'}',0);
	...
	serial2lcd(AUSW_MAIN_NUMBER,'ч',sub_ind1);
	
	if(F_IND_EN)sub_bgnd("ON",'Ц',-1);
	else sub_bgnd("OFF",'Ц',-2);
	}
//o_2_e	
}
...
//o_2_s
	...
	ptrs[0]=" Inverter 24V       ";
	ptrs[1]=" Inverter 48(60)V   ";
	ptrs[2]=" Inverter 110V      ";
	ptrs[3]=" Inverter 220V      ";
	ptrs[4]=" Exit               ";
	if(bFL5)ptrs[default_temp]=sm_;
    ...
    bgnd_par(" STANDARD SETTINGS  ",ptrs[index_set],ptrs[index_set+1],ptrs[index_set+2]);
	}
//o_2_e		
	pointer_set(1); 
	} 
...
//o_2_s
	 ...
	 if(phase==0)ptrs[2]="   љ њ - selection  ";
     if(phase==1)ptrs[2]="   ї ќ - set        ";
     if(phase==2)ptrs[2]="     ¤ - exit       ";	
	bgnd_par("      SET TIME      ",ptrs[0],ptrs[1],ptrs[2]);
} //o_2_e
 	...
 	int2lcd(LPC_RTC->DOM,'<',0);
	if(language){sub_bgnd(sm_mont[LPC_RTC->MONTH],'>',0);} //o_2
	else {sub_bgnd(sm_mont_en[LPC_RTC->MONTH],'>',0);} //o_2
 	int2lcd(LPC_RTC->YEAR,'{',0);
 	if(bFL2)
 	     {
 	     lcd_buffer[find(':')]=' ';
...
//o_2_s
	...
	if(SNTP_ENABLE==0)		ptrs[0]=	" OFF                ";
	else if(SNTP_ENABLE==1)	ptrs[0]=	" Check        1 hour";
	else if(SNTP_ENABLE==2)	ptrs[0]=	" Check         1 day";
	else if(SNTP_ENABLE==3)	ptrs[0]=	" Check        1 week";
	...
	if(SNTP_ENABLE==0)
		...
		{
		if((SNTP_GMT>=0)&&(SNTP_GMT<=9))			ptrs[1]=	" Timezone GMT+!     ";
		else if((SNTP_GMT>=10)&&(SNTP_GMT<=13))		ptrs[1]=	" Timezone GMT+ !    ";
		else if((SNTP_GMT<0)&&(SNTP_GMT>-10))		ptrs[1]=	" Timezone GMT-!     ";
		else if((SNTP_GMT<=-10)&&(SNTP_GMT>=-12))	ptrs[1]=	" Timezone GMT- !    ";
		}
	if(sub_ind<index_set) index_set=sub_ind;
	else if((sub_ind-index_set)>1) index_set=sub_ind-1;
...
//o_2_s
	...
	bgnd_par("     STRUCTURE      ",ptrs[index_set],ptrs[index_set+1],ptrs[index_set+2]);
...
//o_2_s
	...
	ptrs[0]=		" Dry contact №1     ";
	ptrs[1]=		" Dry contact №2     ";
	ptrs[2]=		" Dry contact №3     ";
	ptrs[3]=		" Dry contact №4     ";
	ptrs[NUMSK]=  	" Exit               ";
	ptrs[NUMSK+1]=  "                    ";
	ptrs[NUMSK+2]=	"                    ";
	ptrs[NUMSK+3]=	"                    ";
	...
	if(sub_ind<index_set) index_set=sub_ind;
     ...
     bgnd_par("    SETTINGS DC     ",ptrs[index_set],ptrs[index_set+1],ptrs[index_set+2]);
}
//_17_e		
	pointer_set(1);
	}
...
if(language){//o_2
	...
	ptrs[0]=		" Выходное напряжение";
	ptrs[1]=		" инвертора       !В ";
	ptrs[2]=		" Напряжение выхода  ";
	ptrs[3]=		" максимальное    @В ";
	ptrs[4]=		" Напряжение выхода  ";
	ptrs[5]=		" минимальное     #В ";
	ptrs[6]=		" Напряжение батареи ";
	ptrs[7]=		" включения       {В ";
	ptrs[8]=		" Напряжение батареи ";
	ptrs[9]=		" отключения      }В ";
	ptrs[10]=		" Выход              ";
...
//o_2_s
	...
	ptrs[0]=		" Inverter output    ";
	ptrs[1]=		"   voltage        !V";
	ptrs[2]=		" Maximum output     ";
	ptrs[3]=		"     voltage      @V";
	ptrs[4]=		" Minimum output     ";
	ptrs[5]=		"     voltage      #V";
	ptrs[6]=		" Turn-on battery    ";
	ptrs[7]=		"     voltage      {V";
	ptrs[8]=		" Turn-off battery   ";
	ptrs[9]=		"     voltage      }V";	
	ptrs[10]=		" Exit               ";
...
else if (ind==iByps_sets)
	{
	char sss[10]="abcdef";
	char i/*,i_flag*/;
if(language){//o_2
			 
	ptrs[0]=		" Uac вкл (завыш)    ";
	ptrs[1]=		"                 !В ";
	ptrs[2]=		" Uac откл (завыш)   ";
	ptrs[3]=		"                 @В ";
	ptrs[4]=		" Uac вкл (заниж)    ";
	ptrs[5]=		"                 #В ";
...
//o_2_s
	...
	ptrs[0]=		" Uac turn-on (high) ";
	ptrs[1]=		"                 !V ";
	ptrs[2]=		" Uac turn-off (high)";
	ptrs[3]=		"                 @V ";
	ptrs[4]=		" Uac turn-on (low)  ";
	ptrs[5]=		"                 #V ";
...
//o_2_e		  
	int2lcd(U_NET_ON_MAX,'!',0);
	int2lcd(U_NET_OFF_MAX,'@',0);
	int2lcd(U_NET_ON_MIN,'#',0);
	int2lcd(U_NET_OFF_MIN,'{',0);
	}
...
//o_2_s
	...
	if(sub_ind<index_set) index_set=sub_ind;
     ...
     bgnd_par("   DRY CONTACT N!   ",ptrs[index_set],ptrs[index_set+1],ptrs[index_set+2]);
	
	pointer_set(1);
	int2lcd(sub_ind1+1,'!',0);
	if(sk_stat[sub_ind1]==ssON)sub_bgnd("CLOSED",'@',-2);
	else sub_bgnd("OPEN",'@',0);
	if(!SK_SIGN[sub_ind1])sub_bgnd("CLOSED",'#',-2);
	else sub_bgnd("OPEN",'#',0);
}
//o_2_e
	}		
...
//o_2_s
	...
	ptrs[0]=	" Alarm invertor    @";
    ptrs[1]=	" Alarm DC input    #";
	ptrs[2]=	" Alarm Uout        $"; 
    ptrs[3]=	" Alarm AC input    %";
    ptrs[4]=	" Bypass mode:       ";
	ptrs[5]=	" mains/invertors   ^";
	ptrs[6]=	" Active state       ";
	ptrs[7]=	"       relay       &";
    ptrs[8]=	" Exit               ";
    ptrs[9]=	sm_;
	...
	checkboxing('>',0);
 	checkboxing('^',RELE_SET_MASK[sub_ind1]&(1<<4));
	if(RELE_SET_MASK[sub_ind1]&(1<<15))		sub_bgnd("ON",'&',-1);
	else 									sub_bgnd("OFF",'&',-2);
}
//o_2_e 
	}
...
//	ptrs[7]=		" Uсети.вкл.      ]В ";
	...
	ptrs[0]=		" Uout.AC.max.    !V ";
	ptrs[1]=		" Uout.AC.min.    @V ";
	ptrs[2]=		" Uin.AC.max.     #V ";
	ptrs[3]=		" Uin.AC.min.     $V ";
	ptrs[4]=		" Uin.DC.max.     %V ";
	ptrs[5]=		" Uin.DC.min.     ^V ";
...
0502*/
	...
	ptrs[6]=						" INVERTOR N7        ";
     ...
     ptrs[8]=						" INVERTOR N9        ";
	ptrs[9]=						" INVERTOR N10       ";
     ptrs[10]=						" INVERTOR N11       ";
     ptrs[11]=						" INVERTOR N12       "; 
	ptrs[12]=						" INVERTOR N13       ";
     ptrs[13]=						" INVERTOR N14       ";
     ptrs[14]=						" INVERTOR N15       ";	
	 ptrs[15]=						" INVERTOR N16       ";
	ptrs[16]=						" INVERTOR N17       ";
     ptrs[17]=						" INVERTOR N18       ";
     ptrs[18]=						" INVERTOR N19       ";
	ptrs[19]=						" INVERTOR N20       ";
     ptrs[20]=						" INVERTOR N21       ";
     ptrs[21]=						" INVERTOR N22       "; 
	ptrs[22]=						" INVERTOR N23       ";
     ptrs[23]=						" INVERTOR N24       ";
     ptrs[24]=						" INVERTOR N25       ";	
	 ptrs[25]=						" INVERTOR N26       ";
	ptrs[26]=						" INVERTOR N27       ";
     ptrs[27]=						" INVERTOR N28       ";
     ptrs[28]=						" INVERTOR N29       ";
	ptrs[29]=						" INVERTOR N30       ";
     ptrs[30]=						" INVERTOR N31       ";
     ptrs[31]=						" INVERTOR N32       "; 
	ptrs[32]=						" INVERTOR N33       ";
     ptrs[33]=						" INVERTOR N34       ";
     ptrs[34]=						" INVERTOR N35       ";	
	 ptrs[35]=						" INVERTOR N36       ";
	ptrs[36]=						" INVERTOR N37       ";
     ptrs[37]=						" INVERTOR N38       ";
     ptrs[38]=						" INVERTOR N39       ";
	ptrs[39]=						" INVERTOR N40       ";
     ptrs[40]=						" INVERTOR N41       ";
     ptrs[41]=						" INVERTOR N42       "; 
	ptrs[42]=						" INVERTOR N43       ";
     ptrs[43]=						" INVERTOR N44       ";
     ptrs[44]=						" INVERTOR N45       ";	              
	ptrs[NUMINV]=					" Exit               ";
	ptrs[1+NUMINV]=				"                    ";
	ptrs[2+NUMINV]=				"                    ";
	...
	bgnd_par("INVERTER CALIBRATION",ptrs[index_set],ptrs[index_set+1],ptrs[index_set+2]);
...
*/
	...
	ptrs[0]=	" Uout =    @V       ";
	ptrs[1]=	"   set value Uout   ";
	ptrs[2]=	"  press key љ or њ  "; 
	ptrs[3]=	" Iout =     %А      ";
	if(phase==0)
          {
          ptrs[4]=	"   press key ¤      ";
          ptrs[5]=	"to calibrate Iout=0A";
          }
     else
     	{
          ptrs[4]="   set value Iout   ";
          ptrs[5]="  press key љ or њ  ";     	
     	} 
     	
	ptrs[6]=	" t inv =   ^°C      ";    
	ptrs[7]=	"  set value t inv   ";
	ptrs[8]=	"  press key љ or њ  ";
	ptrs[9]=	" Ubus  =     &V     ";
	ptrs[10]=	"  set value Ubus    ";
	ptrs[11]=	"  press key љ or њ  "; 
	ptrs[12]=	" Umains =    *V     ";
	ptrs[13]=	"  set value Umains  ";
	ptrs[14]=	"  press key љ or њ  "; 
	ptrs[15]=	" Pout  =     (W     ";
	ptrs[16]=	"  set value Pout    ";
	ptrs[17]=	"  press key љ or њ  ";
	ptrs[18]=	" Uin =       [V     ";
	ptrs[19]=	"   set value Uin    ";
	ptrs[20]=	"  press key љ or њ  ";
	ptrs[21]=	" Exit               ";
	ptrs[22]=	sm_;
	ptrs[23]=	sm_;     	     	    
	...
	bgnd_par(" CALIBRATION INV N! ",ptrs[index_set],ptrs[index_set+1],ptrs[index_set+2]);
	}//o_2_e
	pointer_set(1);	
	int2lcd(sub_ind1+1,'!',0);
	int2lcd(inv[sub_ind1]._Uout,'@',1);
...
if(language){//o_2	
	...
	if(NUMBYPASS>1)	bgnd_par(" КАЛИБРОВКА БАЙПАСN!",ptrs[index_set],ptrs[index_set+1],ptrs[index_set+2]);	
	else bgnd_par(" КАЛИБРОВКА БАЙПАС  ",ptrs[index_set],ptrs[index_set+1],ptrs[index_set+2]);
	...
	if(NUMBYPASS>1)	bgnd_par(	" BYPASS CALIBRAT. N!",ptrs[index_set],ptrs[index_set+1],ptrs[index_set+2]);	
	else bgnd_par(				" BYPASS CALIBRATION ",ptrs[index_set],ptrs[index_set+1],ptrs[index_set+2]);
...
if(language){//o_2	
	...
	ptrs[27]=	" UинвA =    )В      ";
	ptrs[28]=	"откалибруйте Uинв   ";
	ptrs[29]=	"  нажатием љ или њ  ";
	ptrs[30]=	" UинвB =    -В      ";
	ptrs[31]=	"откалибруйте Uинв   ";
	ptrs[32]=	"  нажатием љ или њ  ";
	ptrs[33]=	" UинвC =    +В     ";
	ptrs[34]=	"откалибруйте Uинв  ";
	...
	ptrs[0]=	" UoutA =    !V      ";
	ptrs[1]=	"  set value UoutA   ";
	ptrs[2]=	"  press key љ or њ  ";
	ptrs[3]=	" UoutB =    @V      ";
	ptrs[4]=	"  set value UoutB   ";
	ptrs[5]=	"  press key љ or њ  ";
	ptrs[6]=	" UoutC =    #V      ";
	ptrs[7]=	"  set value UoutC   ";
	ptrs[8]=	"  press key љ or њ  ";	
	...
	if(phase==0)
		{
          ptrs[10]=	"   press key ¤      ";
          ptrs[11]=	"to calibrate Iout=0A";
          }
     else
     	{
          ptrs[10]="   set value Iout   ";
          ptrs[11]="  press key љ or њ  ";     	     	
     	} 
	ptrs[12]=		" IoutB =     %А     ";
	if(phase==0)
		{
          ptrs[13]=	"   press key ¤      ";
          ptrs[14]=	"to calibrate Iout=0A";
          }
     else
     	{
          ptrs[13]="   set value Iout   ";
          ptrs[14]="  press key љ or њ  ";     	
     	}
	ptrs[15]=		" IoutC =     ^А     ";
	if(phase==0)
		{
          ptrs[16]=	"   press key ¤      ";
          ptrs[17]=	"to calibrate Iout=0A";
          }
     else
     	{
          ptrs[16]="   set value Iout   ";
          ptrs[17]="  press key љ or њ  ";     	
     	}
	...
	ptrs[21]=	" tB    =   *°C      ";    
	ptrs[22]=	"    set value t     ";
	ptrs[23]=	"  press key љ or њ  ";
	ptrs[24]=	" tC    =   (°C      ";    
	ptrs[25]=	"    set value t     ";
	ptrs[26]=	"  press key љ or њ  ";

	ptrs[27]=	" UinvA =    )V      ";
	ptrs[28]=	"  set value UinvA   ";
	ptrs[29]=	"  press key љ or њ  ";
	ptrs[30]=	" UinvB =    -V      ";
	ptrs[31]=	"  set value UinvB   ";
	ptrs[32]=	"  press key љ or њ  ";
	ptrs[33]=	" UinvC =    +V      ";
	ptrs[34]=	"  set value UinvC   ";
	ptrs[35]=	"  press key љ or њ  ";
			 
	ptrs[36]=	" UmainsA =    {V    ";
	ptrs[37]=	" set value UmainsA  ";
	ptrs[38]=	"  press key љ or њ  "; 
	ptrs[39]=	" UmainsB =    }V    ";
	ptrs[40]=	" set value UmainsB  ";
	ptrs[41]=	"  press key љ or њ  "; 
	ptrs[42]=	" UmainsC =    [V    ";
	ptrs[43]=	" set value UmainsC  ";
	ptrs[44]=	"  press key љ or њ  ";
	 
	ptrs[45]=	" PoutA  =     ]W    ";
	ptrs[46]=	"  set value PoutA   ";
	ptrs[47]=	"  press key љ or њ  "; 
	ptrs[48]=	" PoutB  =     <W    ";
	ptrs[49]=	"  set value PoutB   ";
	ptrs[50]=	"  press key љ or њ  ";
	ptrs[51]=	" PoutC  =     >W    ";
	ptrs[52]=	"  set value PoutC   ";
	ptrs[53]=	"  press key љ or њ  ";
	...
	ptrs[56]=	sm_;     	     	    
 	...
 	bgnd_par(" BYPASS CALIBRATION ",ptrs[index_set],ptrs[index_set+1],ptrs[index_set+2]);
...
/*		int2lcdyx(mat_temper,0,7,0);
		...
		int2lcdyx(byps[0]._mnl_net_cnt,1,16,0);
		int2lcdyx(byps[0]._mnl_net_stat,2,16,0);     		    
...
//o_2_s
	...
	bgnd_par(	"Software version CPU",
				" Date    2000.00.00 ",
				"Software version PLD",
				" Date    2000.00.00 ");
}
//o_2_e
	for(i=0; i<80; i++)
...
/*     else if(sub_ind==14)
 ...
 /*    else if(sub_ind==20)
		...
		if(sub_ind1)
	     ...
	     {
		if(F_IND_EN)F_IND_EN=0;
		else F_IND_EN=1;
	     lc640_write_int(EE_F_IND_EN,F_IND_EN);
	     speed=1;
	     }
 
	else if(sub_ind==20)   //o_2_s
		{
		if(but==butE)
		     {
		     if(language) {language=0; lc640_write_int(EE_LANGUAGE,0); }
			 else {language=1; lc640_write_int(EE_LANGUAGE,1); }
		     }
		}				 //o_2_e
     else if(sub_ind==21)
	     {
	     ...
	     if((but==butR)||(but==butR_))
	     	{
	     	if(KAN_BR==62)KAN_BR=125;
			else KAN_BR=62;
	     	lc640_write_int(EE_KAN_BR,KAN_BR);
			speed=1;
	     	}
...
/*     else if(sub_ind==6)
	     ...
	     if((but==butR)||(but==butR_))
	     	...
	     	U_AC_1++;
	     	gran(&U_AC_1,200,300);
	     	lc640_write_int(EE_U_AC_1,U_AC_1);
			speed=1;
	     	}
 ...
 /*    else if(sub_ind==7)
	     ...
	     if((but==butR)||(but==butR_))
	     	...
	     	U_AC_2++;
	     	gran(&U_AC_2,200,300);
	     	lc640_write_int(EE_U_AC_2,U_AC_2);
			speed=1;
	     	}
...
else if((ind==iFWInv_about) || (ind==iFWByps_about))
	{
	ret(1000);
	if(but==butE)
	     {
