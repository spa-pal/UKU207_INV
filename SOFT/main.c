...
if(++cnt_net_drv>50) 
	{
	cnt_net_drv=0;
	mcp2515_transmit(0xf1,(char)U_OUT_SET,(char)U_OUT_MIN,(char)U_OUT_MAX,(char)U_NET_MIN,(char)U_NET_MAX,(char)U_BAT_MIN,(char)U_BAT_MAX);
	} 
...
if((cnt_net_drv>=MINIM_INV_ADRESS)&&(cnt_net_drv<MINIM_INV_ADRESS+15))
	...
	if(NUMBYPASS)
		...
		if(cnt_net_drv==MINIM_INV_ADRESS)
			{
			if(byps._cnt<CNT_SRC_MAX)byps._cnt++;
			}
		}
...
if(ind==iMn_INV)
 ...
 /*    i=0;
	...
	ptrs[4]=  							" Байпасс            ";     
     ...
     ptrs[4+NUMBYPASS]=  					" Инвертор N1        ";
     ptrs[5+NUMBYPASS]=  					" Инвертор N2        ";
     ptrs[6+NUMBYPASS]=  					" Инвертор N3        ";
     ptrs[7+NUMBYPASS]=  					" Инвертор N4        ";
     ptrs[8+NUMBYPASS]=  					" Инвертор N5        ";
     ptrs[9+NUMBYPASS]=  					" Инвертор N6        ";
     ptrs[10+NUMBYPASS]=  					" Инвертор N7        ";
     ptrs[11+NUMBYPASS]=  					" Инвертор N8        ";
     ptrs[12+NUMBYPASS]=  					" Инвертор N9        ";
     ptrs[13+NUMBYPASS]=  					" Инвертор N10       ";
     ptrs[14+NUMBYPASS]=  					" Инвертор N11       ";
     ptrs[15+NUMBYPASS]=  					" Инвертор N12       ";
     ptrs[16+NUMBYPASS]=  					" Инвертор N13       ";
     ptrs[17+NUMBYPASS]=  					" Инвертор N14       ";
     ptrs[18+NUMBYPASS]=  					" Инвертор N15       ";
     ptrs[19+NUMBYPASS]=  					" Инвертор N16       ";
     ptrs[20+NUMBYPASS]=  					" Инвертор N17       ";
     ptrs[21+NUMBYPASS]=  					" Инвертор N18       ";
     ptrs[22+NUMBYPASS]=  					" Инвертор N19       ";
     ptrs[23+NUMBYPASS]=  					" Инвертор N20       ";
...
/*	int2lcdyx(makb[2]._cnt,0,10,0);*/
	//int2lcdyx(LPC_SC->RSID,0,3,0);	
 	//int2lcdyx(plazma_rx,0,10,0);
	//int2lcdyx(sc16is700ByteAvailable,0,19,0);
...
//av_head_int[1]=456;	
	...
	ptrs[6]=		" инвертора       !В ";
	ptrs[7]=		" Напряжение выхода  ";
	ptrs[8]=		" максимальное    @В ";
	ptrs[9]=		" Напряжение выхода  ";
	ptrs[10]=		" миниимальное    #В ";
	ptrs[11]=		" Напряжение сети    ";
	ptrs[12]=		" максимальное    [В ";
	ptrs[13]=		" Напряжение сети    ";
	ptrs[14]=		" миниимальное    ]В ";
	ptrs[15]=		" Напряжение батареи ";
	ptrs[16]=		" максимальное    {В ";
	ptrs[17]=		" Напряжение батареи ";
	ptrs[18]=		" миниимальное    }В ";
	ptrs[19]=		" Ethernet           ";
	ptrs[20]=		" MODBUS ADRESS     <";
	ptrs[21]=		" MODBUS BAUDRATE    ";
	ptrs[22]=		"                  >0";
    ptrs[23]=		" Выход              ";
    ptrs[24]=		" Калибровки         "; 
    ptrs[25]=		"                    ";        
...
//int2lcdyx(u_necc,0,19,0);  
	...
	bgnd_par("КАЛИБРОВКА БАЙПАСС  ",ptrs[index_set],ptrs[index_set+1],ptrs[index_set+2]);
...
/*		mess_send(MESS2BAT_HNDL,PARAM_BAT_ALL_OFF_AFTER_2SEC,0,10);
	...
	*/
	...
	int2lcd(inv[sub_ind1]._Uio/10,'^',0);
	int2lcd(inv[sub_ind1+1]._Uio/10,'^',0);
	int2lcd(inv[sub_ind1+2]._Uio/10,'^',0);
...
//FullCAN_SetFilter(2,0x18e);
	...
	if(but==butD)
		...
		else if((sub_ind>0)&&(sub_ind<=(0+NUMBYPASS)))
		    	{
		    	tree_up(iByps,0,0,0);
		    	}
		else if((sub_ind>(0+NUMBYPASS))&&(sub_ind<=(0+NUMBYPASS+NUMINV)))
		    	{
		    	tree_up(iInv_v3,0,0,sub_ind-NUMBYPASS-1);
		    	}

		else if(sub_ind==(1+NUMBYPASS+NUMINV))
			{
...
/**/	   	else if(sub_ind==(2+(NUMBAT!=0)+(NUMIST!=0)+(NUMINV!=0)+2))	   /**/
	...
	if(but==butD)
		{
		sub_ind++;
		gran_char(&sub_ind,0,1+NUMBYPASS);
		}
	else if(but==butU)
		{
		sub_ind--;
		...
		if(sub_ind==0)
          ...
          else if((sub_ind==2)||((sub_ind==1)&&(!NUMBYPASS))	)
			{
	          tree_down(0,0);
	          ret(0);
               }	               			
