var set_dict = [
	...
	["Блок дополнительных реле","Наличие или отсутствие в структуре ИБЭП блока расширения релейных выходов","","ONOFF","RADIOINPUT","",["Нет","Есть"],"numbdr_",68],
	["Поэлементный мониторинг АКБ ","Количество блоков поэлементного контроля АКБ в структуре ИБЭП ","","INT","RADIOINPUT","",["0   ", "1   ", "2   ", "3   ","4   "],"nummakb_",63],
	["Внешний шунт нагрузки","Наличие или отсутствие в структуре ИБЭП внешнего шунта нагрузки","","ONOFF","RADIOINPUT","",["Нет","Есть"],"numloadshunt_",67],
	["Средних точек на батарею ","Количество точек контроля разбалансировки в одной группе батарей","","ONOFF","RADIOINPUT","",["1","2"],"nummidpoint_",66],
	/*["Номинальное напряжение батареи","Рабочее напряжение введенной батареи (24В, 48В, 60В)","","INT","RADIOINPUT","",["24В","48В","60В"],"batvolt_",69],*/

	["Выпрямители","HEAD"," "," "," "," ",[ ]," ","s00"],
	["Стартовый шим выпрямителей","Значение (в %) шима выпрямителей при старте системы при перезагрузке, дискретность 1%","%","INT","INTINPUT","DIAP",[1,100],"pwm_start_",65],
	["Uбпс.ке.","Уровень выходного напряжения источников во время контроля емкости, в % от максимального шим, при установке 0% источники блокируются","%","INTOFF","INTINPUT","DIAP",[0,100],"u_bps_ke_",72],
	...
	["Uсигн","Напряжение сигнализирующее разряд АКБ, дискретность 0.1В","В","INT/10","INTINPUT","DIAP",[150,800],"usign_",14],
	...
	["Автоматический выравнивающий заряд","Включенность/выключенность и период запуска автоматического выравнивающего заряда","","ONOFF","RADIOINPUT","",["ВЫКЛ.","1мес.","2мес.","3мес.","6мес.","12мес."],"avzt_",35],


	["Поэлементный контроль","HEAD"," "," "," "," ",[ ]," ","s00"],
	["Номинальное напряжение батареи","Рабочее напряжение введенной батареи (24В, 48В, 60В)","","INT","RADIOINPUT","",["24В  ","48В  ","60В  "],"batvolt_",69],
	["Uсигн.элем.","Напряжение сигнализирующее разряд АКБ при работе с модулями МАКБ, дискретность 0.1В","В","INT/10","INTINPUT","DIAP",[10,1000],"u_makb_sign_",70],
	["Аварийное отклонение","Аварийный уровень отклонения напряжения элемента батареи от среднего значения","%","INTOFF","INTINPUT","DIAP",[0,100],"u_bm_av_",71],
	/*["Параллельная работа выпрямителей","Включенность/выключенность параллельной работы выпрямителей","","ONOFF","RADIOINPUT","",["ВЫКЛ.","ВКЛ."],"par_______",3],*/
	...
	["Сухой контакт №45, звуковой сигнал УКУ","Включенность/выключенность отображения аварийного статуса сухого контакта №4 звуковым сигналом УКУ","","ONOFF","RADIOINPUT","",["ВЫКЛ.","ВКЛ."],"sk4zven_",55],


	["Команды","HEAD"," "," "," "," ",[ ]," ","s00"],
	["Сброс УКУ",'Перезагрузка \("железный" сброс\) УКУ воздействием на сторожевой таймер ',"","","EXECBUTTON","","Произвести сброс","cmd_reset_","sCMD"]
...
function parol_labels(inp_){
	console.log("pFIP   ", passwordFormIsPainted);

	if((inp_=="OFF")||(inp_=="DENIED")){
		console.log("in pFIP   ");
		if(passwordFormIsPainted==0){
			...
			passwordFormIsPainted=1;
		}	
	}
	console.log("pFDIP   ", passwordFormDeniedIsPainted);
	if(inp_=="DENIED"){
		console.log("in pFDIP   ");
